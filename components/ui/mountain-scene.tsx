...
type Props = {
  className?: string;
  /** Hex color of the surface tint. Defaults to brand mint. */
  color?: string;
};

export function MountainScene({ className, color = '#8EF0B5' }: Props) {
  const mountRef = useRef<HTMLDivElement | null>(null);
  const lightPosRef = useRef(new THREE.Vector3(0, 0, 5));

  useEffect(() => {
    const currentMount = mountRef.current;
    if (!currentMount) return;

    // Guard against zero-sized mount on first paint (causes WebGL crashes / NaN aspect).
    const w = Math.max(1, currentMount.clientWidth);
    const h = Math.max(1, currentMount.clientHeight);
    ...
    try {
      ...
      // Browser failed to create WebGL context (e.g. headless or blocked GPU).
      console.warn('[MountainScene] WebGL unavailable, skipping render.', err);
      return;
    }
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        time: { value: 0 },
        pointLightPosition: { value: lightPosRef.current.clone() },
